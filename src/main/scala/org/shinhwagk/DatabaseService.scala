...
object DatabaseService {
  ...
  def queryToAarry(rs: ResultSet): List[List[String]] = {
    ...
    while (rs.next()) {
      ...
      val row = (1 to meta.getColumnCount).toList.map(i => (if (rs.getString(i) == null) "" else rs.getString(i)))
      //      for (i <- 1 to meta.getColumnCount) {
      //        row += (if (rs.getString(i) == null) "" else rs.getString(i))
      //      }
      rows += row.toList
    }
    rows.toList
  ...
  def queryToArray(qi: QueryInfo) = Future {
    val ci = qi.ci
    val sqlText = qi.sqlText
    val parameters = qi.parameters
    val conn = DriverManager.getConnection(ci.jdbcUrl, ci.username, ci.password)
    ...
    val rs = stmt.executeQuery()
    val meta = rs.getMetaData
    val COLUMN_COUNT: Int = meta.getColumnCount


    val rows = new ArrayBuffer[List[String]]()
    rows += (1 to COLUMN_COUNT).toList.map(meta.getColumnName)
    while (rs.next()) {
      val row = new ArrayBuffer[String]()
      for (i <- 1 to COLUMN_COUNT) {
        row += (if (rs.getString(i) == null) "" else rs.getString(i))
